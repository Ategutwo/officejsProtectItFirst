...
export async function run() {
  await Excel.run(async (context) => {
    ...
    if (autoReplenishHistoryUsedRange.rowCount > 1) {
      ...
      if(autoReplenishHistoryUsedRange.columnCount > 1) {
        ...
        // Convert current month to YYYY-MM format for comparison
        const currentMonthKey = currentMonthFormatted.substring(0, 7);
        
        // Find if current month exists in headers (normalize to YYYY-MM for comparison)
        let currentMonthPosition = -1;
        if (headersHistory) {
            for (let i = 0; i < headersHistory.length; i++) {
                const headerMonth = headersHistory[i] ? headersHistory[i].substring(0, 7) : "";
                if (headerMonth === currentMonthKey) {
        ...
        // Get all month keys from the forecast data
        const targetMonths = autoReplenishDatesAndData.map(x => x[0]);
        
        // Build a map of date to row index based on actual month values
        let dateToRowMap = new Map();
        let datesInHistory = datesRange.values ? datesRange.values.map(x => x[0]) : [];
        
        // Normalize dates in history to YYYY-MM format
        for (let i = 0; i < datesInHistory.length; i++) {
            const normalizedDate = normalizeDateForComparison(datesInHistory[i]);
            if (normalizedDate) {
                dateToRowMap.set(normalizedDate, i);
            }
        }
        
        console.log("Date to Row Map:", Array.from(dateToRowMap.entries()));
        console.log("Target Months:", targetMonths);
        
        // Create arrays for writing data
        let writeRows = [];
        let writeValues = [];
        
        // Find which target months exist in the history dates
        for (let i = 0; i < targetMonths.length; i++) {
            const targetMonth = targetMonths[i];
            const rowIndex = dateToRowMap.get(targetMonth);
            
            if (rowIndex !== undefined) {
                // Found matching date, add to write list
                writeRows.push(rowIndex);
                writeValues.push(autoReplenishDatesAndData[i][1]);
            } else {
                console.warn(`Target month ${targetMonth} not found in history dates`);
            }
        }
        
        // Check if current month exists in the headers
        if (currentMonthPosition !== -1) {
            // Add data to the same existing column
            if (writeRows.length > 0) {
                // Write data to the found rows
                for (let i = 0; i < writeRows.length; i++) {
                    const rowOffset = writeRows[i];
                    sheet.getRangeByIndexes(
                        2 + rowOffset, 
                        currentMonthPosition + 1, 
                        1, 
                        1
                    ).values = [[writeValues[i]]];
                }
            }
        } else {
            // Add data to a new column (the last column)
            const newColumnIndex = autoReplenishHistoryUsedRange.columnCount;
            
            // Write header
            sheet.getRangeByIndexes(1, newColumnIndex, 1, 1).numberFormat = [["@"]];
            sheet.getRangeByIndexes(1, newColumnIndex, 1, 1).values = [[currentMonthFormatted]];
            
            // Write values to the correct rows based on month
            if (writeRows.length > 0) {
                for (let i = 0; i < writeRows.length; i++) {
                    const rowOffset = writeRows[i];
                    sheet.getRangeByIndexes(
        ...
        // Sheet has headers but no data columns yet - initialize
        await initializeHistorySheet(sheet, autoReplenishDatesAndData, "AutoReplenishHistory", context);
      }
    } else {
    ...
    // AED AutoReplenishHistory - NOW MATCHES AutoReplenishHistory FORMAT
    if (aedAutoReplenish.size > 0) {
      try {
        ...
        if (aedHistoryUsedRange.rowCount <= 1) {
          // Sheet is empty - initialize it same as AutoReplenishHistory
          await initializeHistorySheet(aedHistorySheet, aedAutoReplenishDatesAndData, "AutoReplenishAEDHistory", context);
        } else {
          // Existing sheet with data - MATCHING AutoReplenishHistory logic
          // Get row 2 headers (same as AutoReplenishHistory)
          if (aedHistoryUsedRange.columnCount > 1) {
            const headerRange = aedHistorySheet.getRangeByIndexes(1, 1, 1, aedHistoryUsedRange.columnCount - 1);
            const datesRange = aedHistorySheet.getRangeByIndexes(2, 0, aedHistoryUsedRange.rowCount - 2, 1);
            headerRange.load("values");
            datesRange.load("values");
            await context.sync();
            
            console.log("Dates in AED History:", datesRange.values);
            const headersHistory = headerRange.values[0]; // Row 2 headers
            let currentMonthFormatted = formatDateWithDay(new Date());
            ...
            // Get all month keys from the forecast data
            const targetMonths = aedAutoReplenishDatesAndData.map(x => x[0]);
            ...
            console.log("AED - Date to Row Map:", Array.from(dateToRowMap.entries()));
            console.log("AED - Target Months:", targetMonths);
            ...
            for (let i = 0; i < targetMonths.length; i++) {
                ...
                if (rowIndex !== undefined) {
                    // Found matching date, add to write list
                    writeRows.push(rowIndex);
                    writeValues.push(aedAutoReplenishDatesAndData[i][1]);
                } else {
                    console.warn(`AED - Target month ${targetMonth} not found in history dates`);
                }
            }
            ...
            if (currentMonthPosition !== -1) {
                ...
                if (writeRows.length > 0) {
                    ...
                    for (let i = 0; i < writeRows.length; i++) {
                        const rowOffset = writeRows[i];
                        aedHistorySheet.getRangeByIndexes(
                            2 + rowOffset, 
                            currentMonthPosition + 1, 
                            1, 
                            1
                ...
                // Add data to a new column (the last column)
                const newColumnIndex = aedHistoryUsedRange.columnCount;
            ...
            // Has rows but no data columns yet - initialize
            await initializeHistorySheet(aedHistorySheet, aedAutoReplenishDatesAndData, "AutoReplenishAEDHistory", context);
          }
        }
        
        await context.sync();
        console.log("AED AutoReplenishHistory updated successfully (matching format)");
      } catch (error) {
    ...
    try {
      ...
      const sheetData = [yearlyHeaders, ...yearlyArray];
      
      // Write to sheet
      yearlyRevenueSheet.getRangeByIndexes(0, 0, sheetData.length, yearlyHeaders.length).values = sheetData;
      
      // Format the header row
      const headerRange = yearlyRevenueSheet.getRangeByIndexes(0, 0, 1, yearlyHeaders.length);
      ...
      if (sheetData.length > 1) {
        const dataRange = yearlyRevenueSheet.getRangeByIndexes(1, 1, sheetData.length - 1, yearlyHeaders.length - 1);
...
// Helper function to initialize a history sheet with proper structure
async function initializeHistorySheet(sheet, datesAndData, sheetName, context) {
  try {
    console.log(`Initializing ${sheetName} sheet...`);
    ...
    // Set up the structure - Row 1 is empty, Row 2 has headers
    const allMonths = [...new Set([...datesAndData.map(x => x[0])])].sort();
    
    if (allMonths.length > 0) {
      // Write the months in column A starting from row 2
      const datesColumn = allMonths.map(month => [month]);
      sheet.getRange(`A2:A${datesColumn.length + 1}`).values = datesColumn;
      
      // Write current date as first column header in row 2, column B
      const currentDateFormatted = formatDateWithDay(new Date());
      sheet.getRange("B2").numberFormat = [["@"]];
      sheet.getRange("B2").values = [[currentDateFormatted]];
      
      // Create a map for quick lookup of values by month
      const valueMap = new Map(datesAndData);
      
      // Write the values in column B starting from row 3, matching each month
      const valuesColumn = allMonths.map(month => {
        const value = valueMap.get(month) || 0;
        return [value];
      });
      sheet.getRange(`B3:B${valuesColumn.length + 2}`).values = valuesColumn;
    }
    
    await context.sync();
    console.log(`${sheetName} initialized successfully`);
  } catch (error) {
    console.log(`Error initializing ${sheetName}:`, error);
  }
}
...
// Improved date normalization that handles both formats consistently
function normalizeDateForComparison(dateInput) {
  if (!dateInput) return null;
  
  let dateStr = dateInput.toString().trim();
  
  // Handle "YYYY-MM" format (what we store)
  if (/^\d{4}-\d{2}$/.test(dateStr)) {
    const [year, month] = dateStr.split('-').map(Number);
    return `${year}-${String(month).padStart(2, '0')}`;
  }
  
  // Handle "YYYY-MM-DD" format (what might be in history)
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
    return dateStr.substring(0, 7); // Return just YYYY-MM
  }
...
function findDateIndexInHistory(datesInHistory, targetDate) {
  ...
  if (!normalizedTarget) {
    console.warn("Invalid target date, using index 0");
    return 0;
  }
  ...
  for (let i = 0; i < datesInHistory.length; i++) {
    ...
    if (normalizedHistory === normalizedTarget) {
      console.log(`Found matching date at index ${i}: ${normalizedHistory} === ${normalizedTarget}`);
      return i;
    }
  }
  
  console.warn(`Target date ${normalizedTarget} not found in history. Available dates:`, 
    datesInHistory.map(d => normalizeDateForComparison(d)).filter(Boolean));
  return 0;
}
